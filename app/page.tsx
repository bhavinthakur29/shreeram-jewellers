...
export default function HomePage() {
  ...
  return (
    <main>
      ...
      {/* Heritage Intro */}
      <section className="relative min-h-[500px] overflow-hidden bg-gradient-to-b from-[#FAF6EE] via-[#F5F0E5] to-[#FAF6EE]">
        ...
        <div className="mx-auto grid max-w-[1280px] min-h-[500px] items-center px-6 py-20 md:grid-cols-2 md:px-20 md:py-0">
      ...
      <section className="mx-auto max-w-[1280px] px-6 py-16 md:px-20">
        ...
        <div className="mt-10 text-center">
          <Link href="/products" className="inline-flex items-center gap-2 rounded-full border border-maroon/20 px-8 py-3.5 font-sans text-[10px] font-medium uppercase tracking-[0.2em] text-maroon transition-all duration-300 hover:bg-maroon hover:text-gold-light hover:border-maroon hover:shadow-[0_0_25px_rgba(200,157,71,0.45)] active:scale-95">
      ...
      <section className="px-6 py-16 md:px-20">
        <div className="mx-auto max-w-[1280px]">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              { icon: '🏛️', title: 'BIS Hallmarked', desc: '916 Gold Certified' },
              { icon: '💎', title: 'GIA Certified', desc: 'Diamond Grading' },
              { icon: '🚚', title: 'Free Delivery', desc: 'Pan India Shipping' },
              { icon: '🔄', title: 'Easy Returns', desc: '7-Day Return Policy' },
            ].map((badge) => (
              <div key={badge.title} className="text-center">
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-maroon/5 text-2xl">{badge.icon}</div>
                <h3 className="mt-4 font-serif text-base text-maroon">{badge.title}</h3>
                <p className="mt-1 font-sans text-[10px] uppercase tracking-[0.15em] text-maroon/40">{badge.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Newsletter */}
      <section className="px-6 py-24 text-center md:px-20 bg-[#F5F0E5]">
        <div className="mx-auto max-w-[560px]">
          ...
          <div className="mt-8 flex gap-3 max-w-md mx-auto">
            <input
              ...
              className="flex-1 h-12 rounded-full border border-[#E5DDD0] bg-white/70 px-5 font-sans text-sm text-maroon outline-none focus:border-gold focus:ring-2 focus:ring-gold/10 transition-all placeholder:text-maroon/30"
            />
            <button className="h-12 rounded-full bg-maroon border border-gold/40 px-6 font-sans text-[10px] font-medium uppercase tracking-[0.2em] text-gold-light transition-all duration-300 hover:bg-maroon-dark hover:shadow-[0_0_25px_rgba(200,157,71,0.45)]">
              Subscribe
            </button>
          </div>
        </div>
