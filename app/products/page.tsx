...
function ProductsContent() {
      ...
      .then((data) => {
        if (Array.isArray(data.products) && data.products.length > 0) {
          const normalized = data.products.map((p: any) => ({
            ...p,
            collection: p.collection || p.category || 'Heritage',
            tags: Array.isArray(p.tags) ? p.tags : [p.category, p.purity].filter(Boolean),
            rating: typeof p.rating === 'number' ? p.rating : 4.9,
            badge: p.badge || (p.inStock === false ? 'Out of Stock' : undefined),
  ...
  const filtered = useMemo(() => {
    let result = category === 'All' ? [...items] : items.filter((p) => p.category === category)
  ...
  }, [items, category, sort, searchQuery])
  ...
  return (
    <main className="mx-auto max-w-[1280px] px-6 pt-32 pb-16 md:px-20 md:pt-36 md:pb-20">
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[280px_1fr]">
        <aside className="hidden bg-white/70 backdrop-blur-lg border border-[#C89D47]/25 rounded-2xl p-6 shadow-[0_8px_24px_rgba(74,14,23,0.04)] lg:block h-fit sticky top-28">
          <div className="flex items-center justify-between">
            <span className="font-sans text-[10px] font-medium uppercase tracking-[0.2em] text-maroon">
              Filter by
            </span>
            <button
              onClick={() => {
              ...
              className="font-sans text-[10px] uppercase tracking-[0.2em] text-gold-dark transition-colors hover:text-maroon"
            >
              Clear All
            </button>
          </div>
          <div className="mt-8 border-t border-[#C89D47]/10 pt-8">
            <p className="font-sans text-[10px] font-medium uppercase tracking-[0.2em] text-maroon/40">
              Category
            </p>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3 font-sans text-sm text-maroon/70">
                ...
                <select
                  ...
                  className="bg-transparent text-maroon outline-none cursor-pointer"
                >
                  {sortOptions.map((opt) => (
          ...
          <div className="mb-8 flex gap-2 overflow-x-auto lg:hidden scrollbar-hide -mx-6 px-6">
            {['All', ...availableCategories].map((item) => (
              <button
                ...
                className={`whitespace-nowrap rounded-full border px-4 py-2 font-sans text-[10px] uppercase tracking-[0.15em] transition-all duration-200 ${category === item
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <button
                ...
                className="mt-6 rounded-full border border-maroon/20 px-6 py-2.5 font-sans text-[10px] uppercase tracking-[0.2em] text-maroon transition-colors hover:bg-maroon hover:text-gold-light"
              >
                View All Heritage
              </button>
            </div>
